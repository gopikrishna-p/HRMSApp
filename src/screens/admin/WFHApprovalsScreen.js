...
const WFHApprovalsScreen = ({ navigation, route }) => {
    ...
    const [activeTab, setActiveTab] = useState(route?.params?.tab || 'pending'); // 'pending', 'history'
    const [selectedDateFilter, setSelectedDateFilter] = useState('all'); // 'all', 'today', 'week', 'month'
    const [showDateFilterModal, setShowDateFilterModal] = useState(false);
    // Deep-link from EmployeeManagement Quick Actions sets `preselectEmployee`
    // to filter both pending and history lists to that one employee. Phase 3.x
    // of ADMIN_EMPLOYEE_PARITY_AUDIT.md (B3).
