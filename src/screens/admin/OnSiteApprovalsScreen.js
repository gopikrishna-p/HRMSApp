...
const OnSiteApprovalsScreen = ({ navigation }) => {
    ...
    const loadRequests = async (isRefresh = false) => {
        try {
            ...
            console.log('📋 Pending On Site requests response:', pendingResponse);
            ...
            console.log('✅ Loaded On Site history requests:', historyData.length);
            ...
            showToast({
                type: 'error',
                text1: 'Error',
                text2: 'Failed to load On Site requests',
            });
        } finally {
    ...
    const handleApproveRequest = async (request) => {
        Alert.alert(
            ...
            `Approve On Site request for ${request.employee_name} from ${formatDate(request.from_date)} to ${formatDate(request.to_date)}?`,
            [
    ...
    const handleRejectRequest = async (request) => {
        Alert.alert(
            'Reject On Site Request',
            `Reject On Site request for ${request.employee_name}?`,
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    text: 'Reject',
    ...
    const processRequest = async (requestId, action, requestData) => {
        try {
            ...
            console.log(`📤 Processing ${action} for On Site request:`, requestId);
            let response;
            if (action === 'approve') {
            ...
            if (isSuccess) {
                ...
                if (NotificationService) {
                    try {
                        await NotificationService.sendOnSiteApprovalNotification?.(
                            action === 'approve',
                            requestData.from_date,
                            action === 'approve' ? 'Your On Site request has been approved' : 'Your On Site request has been rejected'
                        );
                    } catch (error) {
                ...
                showToast({
                    ...
                    text2: backendData.message || `On Site request has been ${action === 'approve' ? 'approved' : 'rejected'}`,
                });

                // If approved, automatically enable On Site for employee
                if (action === 'approve') {
                    try {
                        console.log('🔧 Auto-enabling On Site for employee:', requestData.employee);
                        const enableResponse = await ApiService.enableOnSiteForEmployee(requestData.employee);
                        console.log('✅ Enable On Site response:', enableResponse);
                        ...
                        if (enableResponse.success && (enableData.success === true || enableData.status === 'success')) {
                            showToast({
                                type: 'info',
                                text1: 'On Site Enabled',
                                text2: enableData.message || `On Site has been enabled for ${requestData.employee_name}`,
                            });
                        }
                    } catch (error) {
                        console.warn('⚠️ Failed to auto-enable On Site:', error);
                    }
                }
            } else {
                showToast({
    ...
    const getRequestTypeIcon = (fromDate, toDate) => {
        ...
        if (daysDiff === 0) {
            return { icon: 'calendar-day', color: '#2196F3' };
        } else {
    ...
    const renderRequestItem = ({ item }) => {
        ...
        return (
            <View style={[styles.requestCard, { borderLeftColor: '#2196F3' }]}>
                {/* Header Row */}
                ...
                <View style={styles.requestDetails}>
                    ...
                    {item.client_location && (
                        <View style={styles.infoRow}>
                            <Icon name="map-marker-alt" size={11} color={colors.textSecondary} />
    ...
    const renderEmptyState = () => {
        const message = activeTab === 'pending' ? 'No pending On Site requests' : 'No request history yet';
        const subtitle = activeTab === 'pending' 
            ? 'All On Site requests have been reviewed' 
    ...
    if (loading) {
        return (
            <View style={styles.loadingContainer}>
                ...
                <Text style={styles.loadingText}>Loading On Site requests...</Text>
            </View>
        );
    }
        ...
        <View style={styles.container}>
            ...
            <View style={styles.tabContainer}>
                ...
                >
                    <Icon
                        name="clock"
                        size={14}
                        color={activeTab === 'pending' ? '#2196F3' : colors.textSecondary}
                    />
                ...
                >
                    <Icon
                        name="history"
                        size={14}
                        color={activeTab === 'history' ? '#2196F3' : colors.textSecondary}
                    />
            ...
            {activeTab === 'history' && (
                <View style={styles.filterContainer}>
                    ...
                    >
                        <Icon name="filter" size={14} color="#2196F3" />
                        <Text style={styles.filterButtonText}>{getDateFilterLabel()}</Text>
                        <Icon name="chevron-down" size={12} color="#2196F3" />
                    </TouchableOpacity>
                    ...
                    <View style={styles.modalContent}>
                        ...
                        >
                            <Icon 
                                name="calendar" 
                                size={16} 
                                color={selectedDateFilter === 'all' ? '#2196F3' : colors.textSecondary} 
                            />
                        ...
                        >
                            <Icon 
                                name="calendar-day" 
                                size={16} 
                                color={selectedDateFilter === 'today' ? '#2196F3' : colors.textSecondary} 
                            />
                        ...
                        >
                            <Icon 
                                name="calendar-week" 
                                size={16} 
                                color={selectedDateFilter === 'week' ? '#2196F3' : colors.textSecondary} 
                            />
                        ...
                        >
                            <Icon 
                                name="calendar-alt" 
                                size={16} 
                                color={selectedDateFilter === 'month' ? '#2196F3' : colors.textSecondary} 
                            />
